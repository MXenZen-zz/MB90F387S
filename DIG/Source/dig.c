...
/**
 *	@file		dig.c
 *	@brief		This file contains the sample test program for DIG debugging.
 *	@details	In this test, an LED will blink at a certain interval by means
 *				of the delay function and the DIG functions. The DIG function
 *				acts similarly to a callback function called inside the DIG
...
/* function prototypes */
static T_void ledMODE(T_void);
static T_void ledON(T_void);
static T_void ledOFF(T_void);
...
static T_void ledMODE(T_void)
{
	...
	/* switch task */
	setDIGFunction(&ledOFF);
}

static T_void ledOFF(T_void)
{
	/* toggle LED */
	WriteBitIOPinData(PIN_P12, PIN_LOW);
	/* switch task */
	setDIGFunction(&ledON);
}

static T_void ledON(T_void)
{
	/* toggle LED */
	WriteBitIOPinData(PIN_P12, PIN_HIGH);
	/* switch task */
	setDIGFunction(&ledOFF);
}
