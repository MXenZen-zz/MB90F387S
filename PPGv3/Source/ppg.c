 ...
 *				interrupt and its callback function. Two 16-bit dual PPG
 ...
 *				resolution of 16 bits (for dual PPG channels), any interval
 *				higher than 65536 us (other than a factor of 2048) might not
...
static T_void init(T_void)
{
	...
	/* initialize timer */
	initPPG();
	setupPPGTimer(PPG_CH01, 1000UL, &ppgAFunc);				/* 1 ms interval */
	setupPPGTimer(PPG_CH23, 10000UL, &ppgBFunc);			/* 10 ms interval */
	/* run timer */
	runPPGTimer(PPG_CH01);
	runPPGTimer(PPG_CH23);
