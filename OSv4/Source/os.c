 ...
 *				despite having a specified scheduled interval (also known as
 *				the normal scheduling priority), a task can run indefinitely
 *				by allowing the scheduler to skip checking for its scheduled
 *				interval (also known as the high scheduling priority). This
 *				priority between high and normal scheduling is controlled by
 *				the pin state of the pushbutton of each task. If the pushbutton
 *				is not pressed, the task will be set to execute at high scheduling
 *				priority and the execution function will always run regardless
 *				of the scheduled interval. Conversely, if the pushbutton is
 *				pressed, the task will be set back to execute at normal scheduling
 *				priority and the execution function will run according to its
 *				scheduled interval. In the absence of either initialization
...
static const T_taskProp taskProp1[] = {
	{
		(T_taskID)1U,
		(T_taskName)"Task-10",
		(T_taskPrio)TASK_PRIO(1U),							/* TODO:EXPERIMENT: Change task10's priority if task10's and task11's intervals are the same! */
		(T_taskTime)TASK_SCHED(0UL, 0UL, 500UL),			/* TODO:EXPERIMENT: Change task10's interval (min, sec, millisec) [normal priority only]! */
		...
		(T_taskID)2U,
		(T_taskName)"Task-11",
		(T_taskPrio)TASK_PRIO(2U),							/* TODO:EXPERIMENT: Change task11's priority if task10's and task11's intervals are the same! */
		(T_taskTime)TASK_SCHED(0UL, 1UL, 0UL),				/* TODO:EXPERIMENT: Change task11's interval (min, sec, millisec) [normal priority only]! */
...
TASK(task10Exec)
{
	...
	if (NOT(readDebouncedNegTrig(&swDebPin25, 8U, ReadBitIOPinData(PIN_P25)))) {
		/* set task to execute periodically */
		SetTaskNormalPriority(TASK_THIS);			/* equivalent to SetTaskNormalPriority(task1[0]); */
	} else {
		/* set task to execute all the time */
		SetTaskHighPriority(TASK_THIS);				/* equivalent to SetTaskHighPriority(task1[0]); */
	}
}
...
TASK(task11Exec)
{
	...
	if (NOT(readDebouncedNegTrig(&swDebPin27, 8U, ReadBitIOPinData(PIN_P27)))) {
		/* set task to execute periodically */
		SetTaskNormalPriority(TASK_THIS);			/* equivalent to SetTaskNormalPriority(task1[1]); */
	} else {
		/* set task to execute all the time */
		SetTaskHighPriority(TASK_THIS);				/* equivalent to SetTaskHighPriority(task1[1]); */
	}
}
