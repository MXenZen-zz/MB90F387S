 ...
 *				interrupt and its callback function. Four 8-bit PPG channels
 *				in two 8+8-bit PPG channel configurations are initialized to
 *				execute the callback functions at different interrupt intervals.
 *				Inside the callback function is an interrupt count condition
 *				wherein the LED will toggle if it has reached the right count.
 *				Both callback functions are intended to cause both separately
 *				controlled LEDs to toggle almost simultaneously. Since PPG only
 *				have 1, 2, 4, 8, 16, and 2048 (512/HCLK) divided clocks and a
 *				maximum timer count resolution of 8 bits (for 8+8-bit PPG
 *				channels), any interval higher than 256 us (other than a factor
 *				of 2048) might not be scheduled to interrupt at the right time.
**/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
static T_void init(T_void)
{
	...
	/* initialize timer */
	initPPG();
	setupPPGTimer(PPG_CH01_CH0, 1000UL, &ppgAFunc);			/* 1 ms interval */
	setupPPGTimer(PPG_CH01_CH1, 10000UL, &ppgBFunc);		/* 10 ms interval */
	/* run timer */
	runPPGTimer(PPG_CH01_CH0);
	runPPGTimer(PPG_CH01_CH1);
